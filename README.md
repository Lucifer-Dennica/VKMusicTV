...
**VK Music TV** is a Kotlin Android TV shell with a native, focus-driven dashboard and VK's mobile site available for authenticated catalogue flows. The main screen is designed for a television remote rather than touch input: directional-pad events move focus between large cards, and **OK** opens the selected section. Authentication and the actual audio catalogue remain in VK's own web experience.
...
| D-pad operation | The native dashboard uses large focusable cards. `DPAD_*` changes focus and **OK/Enter** opens the selected section. No virtual cursor is used on the main screen. **Back** follows WebView history or returns to the dashboard. |
