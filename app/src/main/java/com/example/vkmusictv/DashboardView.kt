...
/** Native dashboard supporting both TV D-pad focus and direct touch input. */
class DashboardView(context: Context, private val onAction: (Action) -> Unit) : View(context) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        val top = dp(58f)
        val gap = dp(14f)
        val cardW = (w - 2 * left - 2 * gap) / 3f
        val startY = top + dp(170f)
        val cards = listOf(
            RectF(left, startY, left + cardW, startY + dp(126f)),
            RectF(left + cardW + gap, startY, left + 2 * cardW + gap, startY + dp(126f)),
            RectF(left + 2 * (cardW + gap), startY, left + 3 * cardW + 2 * gap, startY + dp(126f)),
            RectF(left, startY + dp(143f), left + cardW, startY + dp(255f)),
            RectF(left + cardW + gap, startY + dp(143f), left + 2 * cardW + gap, startY + dp(255f))
        )
        val hit = cards.indexOfFirst { it.contains(event.x, event.y) }
        if (hit >= 0) {
            focus = hit
            invalidate()
            onAction(Action.values()[hit])
        }
        return true
    }
