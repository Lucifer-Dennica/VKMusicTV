...
class MainActivity : Activity() {
    ...
    private fun handleDashboardAction(action: DashboardView.Action) {
        when (action) {
            DashboardView.Action.MUSIC, DashboardView.Action.PLAYLISTS, DashboardView.Action.FAVOURITES -> openVk()
            DashboardView.Action.LOGIN -> startVkLogin()
    ...
    private fun startVkLogin() {
        val authLauncher = VK.login(this) { result: VKAuthenticationResult ->
            when (result) {
                is VKAuthenticationResult.Success -> openVk()
                is VKAuthenticationResult.Failed -> openVk()
            }
        }
